...
class TransE(torch.nn.Module):
    def __init__(self, ent_num: int, rel_num: int, dataloader: Dataloader,
                 dim: int = 64, l1: bool = True, margin: float = 1,
                 learning_rate: float = 1e-3, weight_decay: float = 1e-4,
                 device_index: int = 0):
        super().__init__()
        self.device = torch.device(f'cuda:{device_index}') if device_index >= 0 else torch.device('cpu')
    ...
    def forward(self, head, rel, tail) -> torch.Tensor:
        ...
        if self.l1:
            score = torch.sum(torch.abs(torch.add(head_emb, rel_emb) - tail_emb), dim=-1, keepdim=True)
        else:
            score = torch.sum(torch.square(torch.add(head_emb, rel_emb) - tail_emb), dim=-1, keepdim=True)
        return -score
    ...
    def optimize(self, pos, neg):
        ...
        loss = torch.mean(torch.clamp(neg_matrix - pos_matrix + self.margin, min=0))
        return loss
    ...
    def top_k_eval(self, users: List[int], k: int = 5):
        ...
        for user in users:
            ...
            score_ast = np.argsort(scores.cpu().detach().numpy())[::-1]
            sorted_items = []
    ...
    def train_TransE(self, epoch_num: int, output_log=False):
        ...
        for epoch in tqdm(range(epoch_num)):
            ...
            if output_log:
                print(f"The loss after epoch {epoch} is {np.mean(losses)}")
